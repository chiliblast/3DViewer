...
var rimraf = require("rimraf"),
    ...
    uploadedFilesPath = __dirname+"\\Productions",
    chunkDirName = "chunks",
...
function moveUploadedFile(file, uuid, success, failure) {
    var destinationDir = uploadedFilesPath + "/",
        fileDestination = destinationDir + file.originalFilename;

    var temp = fileDestination.split("/");
    fileDestination = "";
    
    for(var i=0; i<temp.length; i++) {
        if(i == 0)
            fileDestination = temp[i];
        else if(i == temp.length-1) {
            destinationDir = fileDestination + "/";
            fileDestination = fileDestination + "/" + temp[i];
        }
        else
            fileDestination = fileDestination + "\\" + temp[i];
    }
